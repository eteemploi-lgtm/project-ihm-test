...
function setConnectedUI(connected) {
    ...
    if (connectionStatus) {
        ...
        connectionStatus.className   = 'status-bar' + (connected ? ' success' : '');
    }
...
function afficherDetailTrameTest(finalView, checksum, ident) {
    ...
    // ── Checksum en hex ───────────────────────────────────────
    const crcHex = Array.from(checksum)
    ...
    // ── Nettoyage de l'ident (supprime les caractères nuls) ───
    const identClean = ident
        .split('')
        .filter(function(c) { return c.charCodeAt(0) !== 0; })
        .join('');
    ...
    // ── Hex dump des 32 premiers octets ───────────────────────
...
function afficherDetailTrameSeq(finalView, checksum, params) {
    ...
    // Champs
    setText('ts_ident',   params.identification.replace(/\x00/g, '') || '—');
