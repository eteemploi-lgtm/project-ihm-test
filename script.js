...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ════════════════════════════════
     ...
     (valeurs réelles des fichiers .cfg)
  ════════════════════════════════ */
  const CFG = {
    moteur: {
      rpmMax:         4428,
  ...
  /* ════════════════════════════════
     TOGGLE SÉRIES
  ════════════════════════════════ */
  const gouvChart = null; // remplacé par schéma SVG
  ...
  /* ════════════════════════════════
     SCHÉMA SVG GOUVERNES — setPale
  ════════════════════════════════ */
  const GOUV_SCALE = 1.33; // degrés réels → rotation SVG visuelle

  function setPale(svgId, valId, angleDeg, pivotX, pivotY) {
    const el  = document.getElementById(svgId);
    const val = document.getElementById(valId);
    if (!el) return;
    const rot = angleDeg * GOUV_SCALE;
    el.setAttribute("transform", "rotate(" + rot + "," + pivotX + "," + pivotY + ")");
    const alertColor = Math.abs(angleDeg) >= 16.5 ? "#f04e68"
                     : Math.abs(angleDeg) >= 13.2 ? "#f5a623" : null;
    if (val) {
      val.textContent = angleDeg.toFixed(1) + "°";
      // Réinitialiser la couleur par défaut selon la gouverne
      const defaults = { valSup:"#a78bfa", valTri:"#56a8ff", valInf:"#4eeaa0", valBab:"#ffc55a" };
      val.setAttribute("fill", alertColor || defaults[valId] || "#dce8f5");
    }
  }

  function updateGouverneSchema(sup, tri, inf, bab) {
    setPale("govSup", "valSup", sup, 240, 140);
    setPale("govTri", "valTri", tri, 340, 240);
    setPale("govInf", "valInf", inf, 240, 340);
    setPale("govBab", "valBab", bab, 140, 240);
  }
  ...
  function renderMotor(m) {
    ...
    const dashMotorRunLed = $("dashMotorRunLed");
    if (dashMotorRunLed) dashMotorRunLed.dataset.state = st==="RUN" ? "ok" : "error";
  ...
  function renderSensors(data) {
    ...
    // LED globale capteurs : ok si tout ok, warn si warn, error si error
    const allStates = [stEau, stPres, stSmo(t1), stSmo(t2), stSmo(t3), stPlaq(tP)];
    const globalSt  = allStates.includes("error") ? "error"
                    : allStates.includes("warn")  ? "warn"  : "ok";
    const dashLedCap = $("dashLedCapteurs");
    if (dashLedCap) dashLedCap.dataset.state = globalSt;
  }
  ...
  function renderBattery(b) {
    ...
    // Dashboard
    // dashBatLevel supprimé du dashboard
    ...
    const dashBatLedEl = $("dashBatLed");
    if (dashBatLedEl) dashBatLedEl.dataset.state = batState;
    // dashBatBadge supprimé
  }
  ...
  function renderGouv(g) {
    ...
    GOUVERNES.forEach(key => {
      ...
      // Col Run dans tableau
      const runTd = $('runGouv' + K);
      if (runTd) runTd.dataset.state = isOk ? "ok" : "error";
    ...
    // gouvChart remplacé par schéma SVG
    ...
    // Mise à jour schéma SVG
    updateGouverneSchema(gouvData.sup.pos, gouvData.tri.pos, gouvData.inf.pos, gouvData.bab.pos);
    syncSliders();
    // Dashboard gouverne LED run
    const dashGouvRunLedEl = $("dashGouvRunLed");
    if (dashGouvRunLedEl) dashGouvRunLedEl.dataset.state = anyAlert ? "error" : "ok";
  ...
  // Synchronisation commande position bouton 2 (dans schéma)
  const gouvPosInput2   = $("gouvPosInput2");
  const sendGouvPosBtn2 = $("sendGouvPosBtn2");
  const cbGouvSup2 = $("cbGouvSup2");
  const cbGouvTri2 = $("cbGouvTri2");
  const cbGouvInf2 = $("cbGouvInf2");
  const cbGouvBab2 = $("cbGouvBab2");

  if (sendGouvPosBtn2) {
    sendGouvPosBtn2.addEventListener("click", async () => {
      const val = parseFloat(gouvPosInput2?.value ?? 0);
      if (isNaN(val) || val < -150 || val > 150) {
        const fb = $("gouvCommandFeedback2");
        if (fb) fb.textContent = "⚠ Position invalide.";
        return;
      }
      // Synchroniser avec les checkboxes du bouton 2
      if (cbGouvs.sup && cbGouvSup2) cbGouvs.sup.checked = cbGouvSup2.checked;
      if (cbGouvs.tri && cbGouvTri2) cbGouvs.tri.checked = cbGouvTri2.checked;
      if (cbGouvs.inf && cbGouvInf2) cbGouvs.inf.checked = cbGouvInf2.checked;
      if (cbGouvs.bab && cbGouvBab2) cbGouvs.bab.checked = cbGouvBab2.checked;
      if (gouvPosInput) gouvPosInput.value = val;
      await sendGouvPosition();
      const fb = $("gouvCommandFeedback2");
      if (fb) fb.textContent = "✓ Consigne " + val + "° envoyée";
    });
  }


  const stopMotorBtn = $("stopMotorBtn");
  if (stopMotorBtn) {
    stopMotorBtn.addEventListener("click", async () => {
      simMotor.rpm_cmd = 0;
      try {
        await fetch(`${API_BASE}/motor/rpm_cmd`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ rpm_cmd: 0 })
        });
      } catch(_) {}
      if (motorCommandFeedback)
        motorCommandFeedback.textContent = "⏹ Arrêt moteur envoyé (0 tr/min)";
      addLog("Stop moteur — consigne 0 tr/min", "warn");
      await refreshAll();
    });
  }


  /* ── Sliders schéma gouverne ── */
  const sliders = [
    { id:"slSchSup", lbl:"lblSchSup", svgId:"govSup", valId:"valSup", px:240, py:140, key:"sup" },
    { id:"slSchTri", lbl:"lblSchTri", svgId:"govTri", valId:"valTri", px:340, py:240, key:"tri" },
    { id:"slSchInf", lbl:"lblSchInf", svgId:"govInf", valId:"valInf", px:240, py:340, key:"inf" },
    { id:"slSchBab", lbl:"lblSchBab", svgId:"govBab", valId:"valBab", px:140, py:240, key:"bab" },
  ];
  ...
  sliders.forEach(s => {
    ...
    el.addEventListener("input", () => {
      ...
      setPale(s.svgId, s.valId, val, s.px, s.py);
      simGouv[s.key].position = val;
      // Envoyer commande API
      fetch(API_BASE + "/gouverne/" + s.key + "/position_cmd", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ position_cmd: val })
      }).catch(() => {});
  ...
  // Synchroniser les sliders depuis renderGouv
  function syncSliders() {
    sliders.forEach(s => {
      const el = $(s.id);
      const lbl = $(s.lbl);
      const pos = gouvData[s.key]?.pos ?? 0;
      if (el)  el.value = Math.max(-16.5, Math.min(16.5, pos));
      if (lbl) lbl.textContent = pos.toFixed(1) + "°";
    });
  }
